...
def get_gt_graph(data_dir, seq):
    ...
    return coords, coord_cols, edges
...
def get_frame_det_test_matrix(gt_graph: 'networkx.Graph', gt_frame: 'np.ndarray', gt_nodes: 'List[int]', comp_graph: 'networkx.Graph', comp_frame: 'np.ndarray', comp_nodes: 'List[int]'):
    det_test_matrix = lil_matrix(len(comp_nodes), len(gt_nodes))
    for i, comp_node_id in enumerate(comp_nodes):
        ...
        for j, gt_node_id in enumerate(gt_nodes):
            ...
            det_test_matrix[i, j] = detection_test(gt_blob_mask, comp_blob_mask)
    return det_test_matrix.tocsr()
...
comp_coords, comp_coord_cols, comp_edges = get_comp_graph('/home/draga/PhD/data/cell_tracking_challenge/Fluo-N2DL-HeLa/', '01')
comp_g = make_network_x_graph(comp_coords, comp_edges)
coords, coord_cols, edges = get_gt_graph('/home/draga/PhD/data/cell_tracking_challenge/Fluo-N2DL-HeLa/', '01')
